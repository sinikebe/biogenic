...
| ping return | glow lobe 3, `Color(0.62,0.55,1.00,1)` | 0.55 × nearness | 17°/13°/10° by `ampulla` tier | attack 35ms, decay 260ms; one per body **that is not in a shadow** per pulse (three-senses.md §1), staggered by `distance / 1250` so a pulse reads as a sweep. **Shares lobe 3 with the `ocellus` beam, loudest wins** |
