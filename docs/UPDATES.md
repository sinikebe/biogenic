...
1. `BuildInfo` (first autoload) captures the binary's own versions from the
   generated [`scripts/build_version.gd`](../scripts/build_version.gd). This
...
```
https://github.com/sinikebe/biogenic/releases/latest/download/manifest.json
```
...
keytool -genkeypair -v \
  -keystore biogenic-release.keystore \
  -alias biogenic \
  -keyalg RSA -keysize 2048 -validity 10950 \
  -dname "CN=Biogenic, O=sinikebe, C=FR"
```

Then add three repository secrets:

```bash
gh secret set ANDROID_KEYSTORE_BASE64  < <(base64 -w0 biogenic-release.keystore)
gh secret set ANDROID_KEYSTORE_PASSWORD  # the store password you chose
gh secret set ANDROID_KEY_ALIAS          # biogenic
```
...
3. `user://` is `%APPDATA%\Godot\app_userdata\Biogenic` on Windows. Delete
